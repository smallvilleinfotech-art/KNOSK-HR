...
-- Seed a staff member's document checklist from document_types matching
-- their track (or track-agnostic ones). Idempotent — safe to call more
-- than once thanks to the unique(staff_id, document_type_id) constraint.
create or replace function generate_staff_documents(p_staff_id uuid)
returns void as $$
declare v_track track_type;
begin
  select track into v_track from staff where id = p_staff_id;
  insert into staff_documents (staff_id, document_type_id, status)
  select p_staff_id, id, 'missing'
  from document_types
  where track is null or track = v_track
  on conflict (staff_id, document_type_id) do nothing;
end;
...
-- matching their track + category. Idempotent per (staff, task_name).
...
begin
  ...
  select p_staff_id, t.id, t.task_name, current_date + 14
  from onboarding_templates t
  where (t.track is null or t.track = v_track)
    and (t.staff_category is null or t.staff_category = v_cat)
    and not exists (
      select 1 from onboarding_tasks ot
      where ot.staff_id = p_staff_id and ot.task_name = t.task_name
    )
  order by t.sequence;
end;
$$ language plpgsql security definer set search_path = public;

-- Fires once, right after a staff row is created: seeds the onboarding
-- checklist AND the document inventory checklist in one shot. This is what
-- makes "add a person" actually produce a working record instead of an
-- empty shell someone has to remember to populate by hand.
create or replace function auto_provision_new_staff()
returns trigger as $$
begin
  perform generate_onboarding_tasks(new.id);
  perform generate_staff_documents(new.id);
  return new;
end;
$$ language plpgsql security definer set search_path = public;

create trigger trg_auto_provision_new_staff
after insert on staff
for each row execute function auto_provision_new_staff();
...
begin
  ...
  select p_staff_id, t.id, t.task_name, t.is_mandatory, current_date + 14
  from offboarding_templates t
  where t.track is null or t.track = v_track
  and not exists (
    select 1 from offboarding_tasks ft
    where ft.staff_id = p_staff_id and ft.task_name = t.task_name
  )
  order by t.sequence;
end;
$$ language plpgsql security definer set search_path = public;

-- Fires the moment a staff record's status changes TO 'offboarding' — no one
-- has to remember to click a "generate tasks" button, the checklist exists
-- automatically from the moment an exit or reassignment-out begins.
create or replace function auto_generate_offboarding_on_status_change()
returns trigger as $$
begin
  if new.employment_status = 'offboarding' and old.employment_status is distinct from 'offboarding' then
    perform generate_offboarding_tasks(new.id);
  end if;
  return new;
end;
$$ language plpgsql security definer set search_path = public;

create trigger trg_auto_offboarding_tasks
after update on staff
for each row execute function auto_generate_offboarding_on_status_change();
...
-- Explicit grants for the provisioning functions (Postgres grants EXECUTE to
-- PUBLIC by default, but being explicit here means this still works even if
-- that default is ever tightened on your project).
grant execute on function generate_onboarding_tasks(uuid) to authenticated;
grant execute on function generate_offboarding_tasks(uuid) to authenticated;
grant execute on function generate_staff_documents(uuid) to authenticated;

-- ----------------------------------------------------------------------------
-- 14c. FILE STORAGE — offer letters, contracts, and other staff documents.
--      Bucket is private; every read goes through a short-lived signed URL
--      generated by the app, never a public link.
-- ----------------------------------------------------------------------------
insert into storage.buckets (id, name, public)
values ('staff-documents', 'staff-documents', false)
on conflict (id) do nothing;

-- Files are stored as staff-documents/<staff_id>/<filename>. Policies key
-- off that first path segment so access lines up with the `staff` RLS rules
-- above: admins/operational roles can read and write anything, everyone
-- else is denied by default (no staff-self-service policy yet — add one
-- deliberately if/when staff should see their own file, so it isn't an
-- accidental byproduct of a generic rule).
create policy staff_documents_storage_read on storage.objects for select
  using (bucket_id = 'staff-documents' and current_app_role() <> 'staff_viewer');

create policy staff_documents_storage_write on storage.objects for insert
  with check (bucket_id = 'staff-documents' and current_app_role() <> 'staff_viewer');

create policy staff_documents_storage_update on storage.objects for update
  using (bucket_id = 'staff-documents' and current_app_role() <> 'staff_viewer');

create policy staff_documents_storage_delete on storage.objects for delete
  using (bucket_id = 'staff-documents' and current_app_role() in ('super_admin','hr_admin'));

-- ----------------------------------------------------------------------------
-- 14d. NOTIFICATIONS
--      The database's job is to reliably decide "this needs to notify
--      someone" — that part has zero external dependencies and cannot fail.
--      Actually sending the email/WhatsApp message is a separate Edge
--      Function (supabase/functions/dispatch-notifications) that reads the
--      pending queue below and requires its own provider credentials
--      (Resend for email, WhatsApp Business API for WhatsApp) — see the
--      README for wiring instructions, since those secrets are yours to
--      provision, not something that can be baked into a schema file.
-- ----------------------------------------------------------------------------
...
-- Queues a notification for every hr_admin/super_admin on every new
-- critical or warning alert, respecting each person's channel preferences
-- (defaulting to "email on" if they haven't set preferences yet).
create or replace function queue_alert_notifications()
returns trigger as $$
begin
  ...
  insert into notification_log (alert_id, profile_id, channel, recipient)
  select new.id, p.id, 'email', p.email
  from profiles p
  left join notification_preferences np on np.profile_id = p.id
  where p.role in ('hr_admin','super_admin')
    and p.is_active
    and coalesce(np.email_enabled, true);

  insert into notification_log (alert_id, profile_id, channel, recipient)
  select new.id, p.id, 'whatsapp', np.whatsapp_number
  from profiles p
  join notification_preferences np on np.profile_id = p.id
  where p.role in ('hr_admin','super_admin')
    and p.is_active
    and np.whatsapp_enabled
...
for each row execute function queue_alert_notifications();

alter table notification_preferences enable row level security;
alter table notification_log enable row level security;

create policy notification_prefs_self on notification_preferences for select
  using (profile_id = auth.uid() or current_app_role() in ('super_admin','hr_admin'));
create policy notification_prefs_self_write on notification_preferences for insert
  with check (profile_id = auth.uid());
create policy notification_prefs_self_update on notification_preferences for update
  using (profile_id = auth.uid())
  with check (profile_id = auth.uid());

-- notification_log is written only by the trigger (security definer) and
-- read/updated only by the Edge Function using the service role key —
-- deliberately no policy grants authenticated clients direct write access.
create policy notification_log_admin_read on notification_log for select
  using (current_app_role() in ('super_admin','hr_admin'));

-- ----------------------------------------------------------------------------
-- 14e. RLS for the remaining reference/lookup tables. These were flagged by
--      Supabase's own "creates tables without RLS" check — correctly: any
--      table without RLS is readable/writable by the anon/authenticated
--      keys by default. Low-sensitivity data (org names, document-type
--      labels, template task names), but there's no reason to leave them
--      open — read access for any signed-in user, writes restricted to
--      the roles that should actually be editing structural setup data.
-- ----------------------------------------------------------------------------
alter table entities enable row level security;
create policy entities_read_all on entities for select using (true);
create policy entities_write_admin on entities for all
  using (current_app_role() in ('super_admin','hr_admin'))
  with check (current_app_role() in ('super_admin','hr_admin'));

alter table document_types enable row level security;
create policy document_types_read_all on document_types for select using (true);
create policy document_types_write_admin on document_types for all
  using (current_app_role() in ('super_admin','hr_admin'))
  with check (current_app_role() in ('super_admin','hr_admin'));

alter table onboarding_templates enable row level security;
create policy onboarding_templates_read_all on onboarding_templates for select using (true);
create policy onboarding_templates_write_admin on onboarding_templates for all
  using (current_app_role() in ('super_admin','hr_admin'))
  with check (current_app_role() in ('super_admin','hr_admin'));

alter table offboarding_templates enable row level security;
create policy offboarding_templates_read_all on offboarding_templates for select using (true);
create policy offboarding_templates_write_admin on offboarding_templates for all
  using (current_app_role() in ('super_admin','hr_admin'))
  with check (current_app_role() in ('super_admin','hr_admin'));
...
-- ============================================================================
-- OPTIONAL: BOOTSTRAP YOUR FIRST LOGIN
-- ============================================================================
-- Everything above is safe to run as-is. This last block is different: it
-- creates one real login (a Supabase auth user + matching admin profile) so
-- you have a working account the moment the schema finishes running,
-- instead of a separate manual step in the dashboard.
--
-- >>> EDIT THE EMAIL AND PASSWORD BELOW BEFORE RUNNING <<<
-- Then sign in with them and change the password immediately from inside
-- the app (Supabase Auth → Users → your account → reset password), since
-- it will otherwise sit in your SQL editor history in plain text.
--
-- This writes directly to Supabase's internal auth.users table, which isn't
-- official public API — it's a widely-used, currently-reliable pattern for
-- seeding a first user, but if a future Supabase version changes that
-- table's shape and this block errors, just create the user normally
-- instead (Dashboard → Authentication → Users → Add user), then run ONLY
-- the second insert below (into `profiles`), swapping in the real user id.
-- ============================================================================
do $$
declare
...
begin
  if exists (select 1 from auth.users where email = v_admin_email) then
    raise notice 'A user with email % already exists — skipping creation.', v_admin_email;
    return;
  end if;
  ...
  values (v_admin_id, v_admin_name, v_admin_email, 'super_admin');

  raise notice 'Bootstrap admin created: % — sign in, then change this password.', v_admin_email;
end $$;
